...
func (e WalletEdges) AccountOrErr() (*Account, error) {
	if e.Account != nil {
...
func (*Wallet) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case wallet.FieldIsActive, wallet.FieldDeleted:
		...
		case wallet.ForeignKeys[0]: // account_wallets
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case wallet.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field account_wallets", value)
			} else if value.Valid {
				w.account_wallets = new(int64)
				*w.account_wallets = int64(value.Int64)
			}
		default:
...
func (w *Wallet) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", w.Deleted))
	builder.WriteString(", ")
